...
def train_model(model, criterion, optimizer, scheduler, num_epochs, per_tag, output_dir):
            ...
            if phase=='train':
                ...
                if epoch_acc > best_acc:
                    ...
                    if early_stop_counter>=patience:
                        ...
                        torch.save(model.state_dict(), os.path.join(output_dir, f"efficientnet_per{per_tag}.pth"))
                        return
    ...
    plt.title('EfficientNet Loss'); plt.xlabel('Epoch'); plt.ylabel('Loss'); plt.legend()
    plt.subplot(1,2,2); plt.plot(train_accuracies,label='Train Acc'); plt.plot(val_accuracies,label='Val Acc')
    plt.title('EfficientNet Accuracy'); plt.xlabel('Epoch'); plt.ylabel('Accuracy'); plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"efficientnet_per{per_tag}_curve.png")); plt.close()
...
if __name__=='__main__':
    ...
    output_dir="/data_hdd/lzh/hair_second/ViT_EfficientNet/20250808/efficientnet_hyper"
    ...
    for target in target_dirs:
        ...
        print(f"\n==== Training EfficientNet on {target} ====")
